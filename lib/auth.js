...
export function requireAuthentication(Component) {
  return class AuthenticatedComponent extends React.Component {
    ...
    isAuthenticated2() {
      const { profile, router } = this.props;
      const unPrivate = ['/upload-candidate/e93fc859-8c27-4d29-a540-fcefbe545f85'];
      return get(profile, 'data.token', '') || unPrivate.includes(router.pathname);
    }
      ...
      return (
        <div>
          {this.state.isLoading ? (
            ...
            <div>
              {this.isAuthenticated() || !this.isAuthenticated2() ? <Component {...this.props} /> : loginErrorMessage }
            </div>
          )}
        </div>
