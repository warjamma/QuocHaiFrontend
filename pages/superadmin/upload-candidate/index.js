...
import {
  createCandidateNoAddJob,
  uploadRequest,
  referCandidateForCompany,
  createCandidateAddJob,
...
function UploadCandidate(props) {
  ...
  const onFinish = async (value) => {
    ...
    if (data.cv) {
      dispatch(createCandidateAddJob({ ...initForm, ...data }, job_id)).then(
        (res) => {
          if (res.status) {
            return message
              .success('Create candidate successfully')
          ...
          <Col span={6}>
            ...
            >
              ...
              <Form.Item {...tailLayout}>
                ...
                <Button
                  className="btn-cance"
                  onClick={() => Router.push(`/superadmin/candidates_list`)}
                  htmlType="button"
                >
                  Hủy
                </Button>
