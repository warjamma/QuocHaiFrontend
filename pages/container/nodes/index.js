...
function MyReferred(props) {
        ...
        <div role="presentation" className="custom-company">
          ...
          <div className="info-required">
            <b
              ...
              onClick={() => Router.push(`/node-detail`)}
            >
              {get(record, 'nodeID')}
            </b>
          </div>
  ...
  const handleFilter = async () => {
    ...
    clone.offset = 0;
    setQuery(clone);
    await dispatch(getListNode(clone));
  };
  ...
  const resetSearch = async () => {
    setQuery(initQuery);
    await dispatch(getListNode(initQuery));
  };
  ...
  return (
    <div className="my-referred-container">
      ...
      <Table
        ...
        rowKey="id"
        columns={columns}
        dataSource={get(referred, 'list_node.data', [])}
        // pagination={{
