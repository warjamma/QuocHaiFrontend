...
function MyReferred(props) {
  ...
  const columns = [
      ...
      render: (text, record, index) => (
        <div role="presentation" className="custom-company">
          <div className="logo-company">
            <img
              src={
                get(record, 'company.avatar') === null
                  ? '/default-avatar.png'
                  : get(record, 'company.avatar')
              }
              alt="avatar"
            />
          </div>
          <div className="info-required">
            <b
              ...
              onClick={() => Router.push(`/job-detail/${record.job.id}`)}
            >
      ...
      render: (text, record, index) => (
        ...
        >
          {get(record, 'job', {}).job_role.map((item) => (
            <b className="name-role" key={item} color="blue">
              {item}
            </b>
          ))}
        </div>
    ...
    },
    {
      ...
      align: 'center',
      render: (text, record, index) => (
        <Tag color="blue">{get(record, 'job.reward')}$</Tag>
      ),
    },
    {
      ...
      render: (text, record, index) => (
        <div>
          {get(record, 'on_boarding_at')
            ? moment(get(record, 'on_boarding_at', '')).format('DD-MM-YYYY')
            : 'N/A'}
        </div>
      ),
    },
        ...
        <div>
          {get(record, 'status', '') === 'interview_failed' || get(record, 'status', '') === 'probation_failed'
              ? <Popover content={<div>
                ...
                <Tag color={renderColorTag(record.status)}>
  ...
  return (
    <div className="my-referred-container">
      <div className="header">
        <div>{`Hồ sơ của bạn (${get(
          referred,
          'list_referred.extra_data.total',
          0
        )})`}</div>
      </div>
      <Form
